...
class Dataset(object):
    ...
    def __init__(self, seq_datasets, graph_datasets, batchSize, 
        ...
        self.node = graph_datasets['src_index']
        ...
        self.node_index = graph_datasets['src_index']
    ...
    def _update_data(self):
        ...
        self.node = [self.node[idx] for idx in self.idxs]
        if self.tgt:
    ...
    def __getitem__(self, index):
        ...
        tgtBatch = None
        nodeBatch, node_lens = self._batchify(self.node[index * self.batchSize: (index + 1) * self.batchSize],
                                           align_right=False, include_lengths=True)
        if self.tgt:
        ...
        nodeIndexBatch = [self.node_index[i] for i in idxBatch]
        ...
        node_lens = torch.LongTensor(node_lens).view(1, -1).to(self.device)
        indices = range(len(srcBatch))
        
        rst = {}
        rst['indice'] = indices
        rst['src'] = (wrap(srcBatch), lengths)
        rst['node_src'] = (wrap(nodeBatch), node_lens)
    ...
    def shuffle(self):
        ...
        def shuffle_group(start, end, NEW):
            ...
            RAW = [self.src[start:end], self.tgt[start:end], self.idxs[start:end],self.node_index[start:end], self.node[start:end]]
            DATA = list(zip(*RAW))
            index = torch.randperm(len(DATA))

            src, tgt, idx, node_idx, node = zip(*[DATA[i] for i in index])
            NEW['SRCs'] += list(src)
            NEW['TGTs'] += list(tgt)
            NEW['IDXs'] += list(idx)
            NEW['NODEIDXs'] += list(node_idx)
            NEW['NODEs'] += list(node)
        ...
        NEW = {'SRCs':[], 'TGTs':[], 'IDXs':[], 'NODEIDXs':[], 'NODEs':[]}
        if self.copy:
        ...
        self.src, self.tgt, self.idxs, self.node_index, self.node = NEW['SRCs'], NEW['TGTs'], NEW['IDXs'], NEW['NODEIDXs'], NEW['NODEs']
        #print(self.src[1])
        #print(self.node_index[1])
        if self.copy:
