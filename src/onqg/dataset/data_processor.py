...
def preprocess_batch(batch, n_edge_type, sparse=True, feature=False, dec_feature=0, 
    ...
    """
    inputs = {'seq-encoder':{},'node-encoder':{}, 'graph-encoder':{}, 'encoder-transform':{}, 
    ...
    #print(lengths.size())
    inputs['seq-encoder']['index'] = []
    ###===== Node encoder =====###
    src_node_seq = batch['node_src']
    src_node_seq, node_lengths = src_node_seq[0], src_node_seq[1]
    inputs['node-encoder']['src_seq'], inputs['node-encoder']['lengths'] = src_node_seq, node_lengths
    #print(lengths.size())
    inputs['node-encoder']['index'] = []
    ###===== encoder transform =====###
    ...
    inputs['graph-encoder']['type'] = batch['node_feat'][0][0]  #暂时没有使用TYPE
