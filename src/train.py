...
def main(opt, logger):
  ...
  if opt.seed > 0:
      torch.manual_seed(opt.seed)  #设置随机种子，确保每次随机生成都是固定的数据
  if opt.gpus:
      if opt.cuda_seed > 0:
  ...
  opt.node_feat_vocab = [fv.size for fv in graph_vocabularies['feature']] if opt.node_feature else None
  ...
  #build_model(opt, device, checkpoint=checkpoint)
  del checkpoint
...
if __name__ == '__main__':
  ...
  log_file_name = time.strftime("%Y%m%d-%H%M%S") + '.log.txt'   #返回本地时间
  if opt.log_home:
