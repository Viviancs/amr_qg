...
class RNNEncoder(nn.Module):
    ...
    def __init__(self, n_vocab, d_word_vec, d_model, n_layer,
        ...
        self.word_emb = nn.Embedding(n_vocab, d_word_vec, padding_idx=Constants.PAD) #参数分别是：总次数、表示单词的向量维度、padding数字
    ...
    def forward(self, inputs):
        ...
        #print(enc_output.size()) #torch.Size([32, 64, 512])
        #print(hidden.size()) #torch.Size([2, 32, 256])
...
class GraphEncoder(nn.Module):
    ...
    def forward(self, inputs):
        ...
        if self.feature:         
            ...
            #print(feat_hidden.size())
            s = torch.cat((node_output, feat_hidden), dim=-1)
            #print(s.size())
    ...
    def forward(self, inputs, max_length):
        ...
        for sample_idx, indexes in enumerate(indexes_list):
            ...
            for indexes_idx in indexes:
                ...
                initial_h = seq_output[sample_idx][0].repeat(1).fill_(0)
                bag = pad(torch.stack([initial_h for idx in indexes_idx], dim=0), 
