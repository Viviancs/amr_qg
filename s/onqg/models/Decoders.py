...
from onqg.models.modules.Attention import ConcatAttention, DualAttention
...
class RNNDecoder(nn.Module):
    ...
    def __init__(self, n_vocab, ans_n_vocab, d_word_vec, d_model, n_layer, n_rnn_enc_layer, 
        ...
        #self.word_emb = nn.Embedding(n_vocab, 256, padding_idx=Constants.PAD)
    ...
    def forward(self, inputs, max_length=300):
        ...
        graph_hidden = inputs['graph_hidden']
        ...
        #graph_context = self.decInit(graph_hidden).unsqueeze(0)
        # ans_words = torch.sum(self.ans_emb(ans_seq), dim=1)
        # hidden = self.decInit(hidden).unsqueeze(0)
        ...
        tag = False
        ######
        bsz, trg_seq = tgt_seq.size()
        triangle_mask = torch.triu(torch.ones(trg_seq, trg_seq).to(self.device), diagonal=1)
        triangle_mask.masked_fill_(triangle_mask == 1, -1e20)
        triangle_mask = triangle_mask.repeat(bsz, 1, 1)
        
        
        #print("tgt:", dec_input.size())
        ...
        #cur_graph = self.attn_init(graph_hidden)
...
class DecoderTransformer(nn.Module):
    ...
    def match_seq_node(self, graph_hidden_states, seq_output, node_index, seq_index):
        #输入全连接层并池化
        #graph_fc = []
        #fc = torch.nn.Linear(graph_hidden_states.size(1),seq_output.size(1))
        #for graph_output in graph_hidden_states:
        #  graph_fc.append(fc(graph_output))
        #print(graph_fc.size()) 

        output = torch.zeros(seq_output.size(0),seq_output.size(1),seq_output.size(2))
        for num, i in enumerate(seq_index):
          idx =   self.find(i, node_index)
          #print(idx)
          if idx >= 0:
            output[:,num,:] = graph_hidden_states[:,idx,:]
        return output


    def forward(self, inputs):
        seq_index = inputs['seq_index']
        ...
        if self.layer_attn:
            ...
            graph_hidden_states = [torch.full((batch_size, graph_length, dim_graph_enc), 1e-8).to(self.device) for _ in range(len(graph_output))]
        else:
        ...
        #print(graph_hidden_states.size())
        

        #graph_hidden_states = self.match_seq_node(graph_hidden_states, seq_output, node_index, seq_index)
