...
class Dataset(object):
    ...
    def shuffle(self):
        ...
        def shuffle_group(start, end, NEW):
            ...
            RAW = [self.src[start:end], self.tgt[start:end], self.idxs[start:end],self.node_index[start:end]]
            DATA = list(zip(*RAW))
            index = torch.randperm(len(DATA))

            src, tgt, idx, node_idx = zip(*[DATA[i] for i in index])
            NEW['SRCs'] += list(src)
            NEW['TGTs'] += list(tgt)
            NEW['IDXs'] += list(idx)
        ...
        self.src, self.tgt, self.idxs, self.node_index = NEW['SRCs'], NEW['TGTs'], NEW['IDXs'], NEW['NODEIDXs']
        #print(self.src[1])
