...
def get_data(dataset):
  ...
  for nodes, in_neigh_indices, in_neigh_edges, out_neigh_indices, out_neigh_edges, sources, sentences, answer in dataset: 
    src.append(sources)
    tgt.append(sentences)
    #if answer:
    ans.append(answer)
    node.append(nodes)
    #print(sentences)

    node_indexes, ans_tags, pos_tags, tgt_tags = [], [], [], []
  ...
  data_list = [src, tgt, ans, node]
  rst = {'src':data_list[0], 'tgt':data_list[1], 'amr_node': data_list[3]}
  rst['ans'] = data_list[2]  #if answer:
...
def sequence_data(tr_dataset, va_dataset):
  ...
  print(src_vocab.size())
  print(tgt_vocab.size())
  feats_vocab = []
  ...
  train_feat_idxs = [convert_word_to_idx(feat, vocab, lower=False)[0] for feat, vocab in zip(train_feats, feats_vocab)] if opt.feature else None
  valid_feat_idxs = [convert_word_to_idx(feat, vocab, lower=False)[0] for feat, vocab in zip(valid_feats, feats_vocab)] if opt.feature else None
